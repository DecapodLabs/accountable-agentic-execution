...
Study E is observational. A checked-in historical snapshot pinned to Decapod \texttt{e7df80d} derives 2,127 commits, 326 tags, 612 merge commits, two explicit revert-subject commits, the February--July 2026 development interval, authorship counts, and changed top-level path frequencies from git. It remains an explicitly older observational dataset than the \texttt{207e9a3} implementation audit. These facts establish sustained repository activity, not a treatment effect, correctness, low review burden, or proof that named workbenches produced particular commits. Author-reported workbench use must be labeled separately. The experience report motivates controlled studies but cannot enter their estimates.
