...
An authenticated audit also verified Decapod governance evidence in 16 repositories across the `alexhraber`, `worldofgeese`, and `DecapodLabs` namespaces (13 public and three private): 15 expose `.decapod/config.toml`, while the public [`alexhraber/alexhraber`](https://github.com/alexhraber/alexhraber) profile repository exposes a generated Decapod `AGENTS.md`. The [repository-use inventory](adoption-inventory.md) records the names, visibility, and boundary. Because the repositories are maintainer-correlated, the count is deployment-footprint evidence rather than 16 independent adoptions.
